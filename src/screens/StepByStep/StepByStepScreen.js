...
export default function StepByStepScreen({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    if (stepIndex === 2 && Array.isArray(step.subSteps)) {
      ...
      const fullText = `${step.title || ""}. ${step.description || step.subText || ""
        }`;
      speakText(fullText);
    }
  ...
  const handleNext = () => {
    ...
    if (stepIndex === 1) {
      console.log("Đang ở bước giải thích phép tính hoặc chuẩn bị chia");
    ...
    if (operator === "÷" && stepIndex === 2) {
      ...
      if (columnStepIndex === totalSteps - 1) {
        console.log("Đã hiện xong các bước chia → chuyển bước");
        setStepIndex((prev) => prev + 1);
        return;
      }
          ...
          <TouchableOpacity
            onPress={() => {
              ...
              if (stepIndex === 0) {
                ...
                const speechText = `${currentStep.title || ""}. ${currentStep.description || ""
                  }`;
